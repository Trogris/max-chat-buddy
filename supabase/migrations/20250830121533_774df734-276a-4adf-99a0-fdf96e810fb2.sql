-- Global AI model setting used by all users
...
CREATE POLICY IF NOT EXISTS "Anyone authenticated can view AI settings"
...
CREATE POLICY IF NOT EXISTS "Admins can update AI settings"
ON public.ai_settings
FOR UPDATE
...
CREATE POLICY IF NOT EXISTS "Admins can insert AI settings"
ON public.ai_settings
FOR INSERT
TO authenticated
WITH CHECK (is_admin(auth.uid()));
